...
async function assumeRole(params) {
  ...
  if (isDefined(webIdentityToken)) {
    ...
    try {
      assumeRoleRequest.WebIdentityToken = await fs.promises.readFile(
      ...
      throw new Error(
        `Web identity token file could not be read: ${error.message}`
      );
    }
  }
...
async function getWebIdentityToken() {
  ...
  assert(
    [ACTIONS_ID_TOKEN_REQUEST_URL, ACTIONS_ID_TOKEN_REQUEST_TOKEN].every(
      isDefined
    ),
    "Missing required environment value. Are you running in GitHub Actions?"
  );
...
async function run() {
  ...
  try {
    ...
    const roleSkipSessionTaggingInput =
      core.getInput("role-skip-session-tagging", { required: false }) ||
      "false";
